...
def download_music(hash_id, album_id,rank):
    ...
    with open( path + "/" + audio_name + ".mp3", 'wb') as f:
        f.write(resp.content)
    print(f'{audio_name} 已经下载成功！')

def get_rank_data():
    '''先获取全部榜单的名字跟链接 再根据获取的链接下载榜单歌曲'''
    rank_url = 'https://www.kugou.com/yy/html/rank.html'
    rank_resp = requests.get(rank_url, headers=headers)
    rank_html = rank_resp.text
    rank_html = etree.HTML(rank_html)
    rank_title = rank_html.xpath('//div[3]/div/div[1]//li/a/text()') # 修改为获取span标签内的文本
    rank_title = [title.strip() for title in rank_title if title.strip()]  # 去除空格和空字符串  获取榜单名字
    rank_href = rank_html.xpath('//div[3]/div/div[1]//li/a/@href')   # 获取榜单的url
    # print(rank_title, rank_href)
    for rank,url in zip(rank_title,rank_href):
        print("下载榜单---", rank)
        print("榜单链接---",url)
        rank_url = url
        rank_resp = requests.get(rank_url, headers=headers)
        rank_html = rank_resp.text
        # print(rank_html)
        # 提取出该页面的所有音乐的信息，提炼每首歌的Hash、album_id
        # 使用正则表达式匹配信息
        hash_list = re.findall('"Hash":"(.*?)".*?"album_id":(.*?),', rank_html)
        # print(hash_list)
        # 写个线程，有33个榜单下载完有点慢 创建线程
        thread_list = []
        for hash_id, album_id in hash_list:
            thread = Thread(target=download_music, args=(hash_id, album_id,rank))
            # download_music(hash_id, album_id,rank)
