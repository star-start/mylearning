...
def gen_code():
    ...
    s = "123456789abcdefg"
    crypt_str = t + r + s + t
    n = t + "fTN2pfuisxTavbTuYVSsNJHetwq5bJvCQkjjtiLM2dCratiA" + t
    #使用hashlib完成加密流程
    s1 = hashlib.sha256()
    s2 = hashlib.sha256()
    ...
    data_n = s2.hexdigest().upper()

    return t,data_code,data_n

# def gen_wif():
#
#     t_wif = str(int(time.time()))
#     # n = "0C5B728B2ADD8406FBA1CFF89106B67B39FAA255CD51216CA78B285A2D37910A"
#     crypt_str = t_wif + "fTN2pfuisxTavbTuYVSsNJHetwq5bJvCQkjjtiLM2dCratiA" + t_wif
#     #使用hashlib完成加密流程
#     s1 = hashlib.sha256()
#     #使用加密对象的update方法即可进行加密，注意加密的字符串需要转为字节码
#     #即Bytes类型
#     s1.update(crypt_str.encode())
#     #获取加密的字符串
#     data_wif = s1.hexdigest().upper()
#
#     return t_wif,data_wif


def get_risk_data():
    """获取全国高低风险区域的信息"""
    t,data_code,data_n = gen_code()
    # t_wif,data_wif = gen_wif()
    ...
    headers.update({
        ...
        # "x-wif-signature":data_wif,
        # "x-wif-timestamp":t_wif
        "x-wif-signature": data_n,
        "x-wif-timestamp": t
    })
    #构建请求参数
    data = {
    ...
    if risk_resp.status_code == 200:
        ...
        utime = risk_data['end_update_time']  # 更新时间
        # 低高风险地区表分开，然后整合
        ...
        for risk_type, risk_list in combine_risk:
            for rlist in risk_list:
                ...
                for address in rlist['communitys']:
                    print([utime, province, city, county, address, risk_type])
